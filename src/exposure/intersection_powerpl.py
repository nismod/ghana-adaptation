...
def main():
    ...
    network_path = os.path.join(
        base_path, 'data', 'infrastructure', 'GHA_powerplant.shp')
    ...
    for hazard_path in hazard_paths:
        ...
        csv_fname = os.path.join(
            base_path, 'results', 'exposure', f"pplants__{hazard_id}.csv")

        with open(csv_fname, 'w') as fh:
            w = csv.DictWriter(fh, fieldnames=('pplant_id', 'hazard_id', 'name', 'length', 'geom'))
            w.writeheader()
            ...
            for hazard_n, hazard in enumerate(hazard_df.itertuples()):
                ...
                for hazard_geom in geoms:
                    ...
                    potential_pplants = network_df.iloc[
                        list(network_df.sindex.intersection(hazard_geom.bounds))]
                    print("found", len(potential_pplants), "powerplants")

                    if len(potential_pplants):
                        for pplant in potential_pplants.itertuples():
                            print(pplant.ID, hazard_n)
                            if pplant.geometry.intersects(hazard_geom):
                                print("intersects")
                                intersection_geom = pplant.geometry.intersection(hazard_geom)
        ...
        fname = os.path.join(
            base_path, 'results', 'exposure', f"pplants__{hazard_id}.gpkg")
