...
def main():
    ...
    network_paths = glob.glob(
        os.path.join(base_path, 'data', 'proximity_datasets', '*.gpkg'))
    ...
    for hazard_path in hazard_paths:
        ...
        for network_path in network_paths:
            ...
            csv_fname = os.path.join(
                base_path, 'results', 'exposure', f"{network_id}_{hazard_id}.csv")

            with open(csv_fname, 'w') as fh:
                w = csv.DictWriter(fh, fieldnames=(edge_id, 'hazard_id', 'name', 'length', 'geom'))
                w.writeheader()
                        ...
                        if len(potential_networks):
                            for network in potential_networks.itertuples():
                                ...
                                if network.geometry.intersects(hazard_geom):
                                    ...
                                    w.writerow({
                                        'network_id': getattr(network,edge_id),
                                        'hazard_id': hazard_n,
         ...
         # Write intersection data
            fname = os.path.join(
                base_path, 'results', 'exposure', f"{network_id}_{hazard_id}.gpkg")
